...
namespace Engine
{
	namespace Network
	{
		...
		public:
			...
			friend int GetSocketHandle(Socket * base_socket);
		...
		int GetSocketHandle(Socket * base_socket) { return static_cast<UnixSocket *>(base_socket)->handle; }
		...
		{
			return string(EntityAddress) + L" \"" + EntityName + L"\" " + (EntityDomain == SocketAddressDomain::IPv4 ? L"IPv4" : L"IPv6");
		}
	}
}