			...
			namespace BinaryLoader
			{
				...
				void LoadFont(Font * Source, uint8 * Data, uint Size, IResourceLoader * ResourceLoader, InterfaceTemplate & Result)
				{
					...
					if (!Result.Font.ElementPresent(Name)) {
						...
						SafePointer<IFont> New = ResourceLoader->LoadFont(FontFace, Source->HeightA + int(Source->HeightZ * CurrentScaleFactor), Source->Weight, (Source->Flags & 0x01) != 0, (Source->Flags & 0x02) != 0, (Source->Flags & 0x04) != 0);
						if (New.Inner()) {
