...
namespace Engine
{
	namespace UI
	{
		namespace Controls
		{
			...
			void ComboBox::Render(IRenderingDevice * device, const Box & at)
			{
				Shape ** back = 0;
			...
			void ComboBox::ComboListBox::Render(IRenderingDevice * device, const Box & at)
			{
				if (!_owner) return;
				if (!_view) {
			...
			void TextComboBox::Render(IRenderingDevice * device, const Box & at)
			{
			...
			void TextComboBox::TextComboListBox::Render(IRenderingDevice * device, const Box & at)
			{
				if (!_owner) return;
				if (!_view) {
					if (_owner->ViewDropDownList) {
