...
namespace Engine
{
	namespace UI
	{
		...
		void Control::Render(IRenderingDevice * device, const Box & at) {}
		...
		IRenderingDevice * Control::GetRenderingDevice(void) { return __system ? __system->GetRenderingDevice() : 0; }
			...
			if (system._preferred_device_class != Windows::DeviceClass::Null) {
				if (system._engine) {
					...
					if (system._engine->BeginRenderingPass()) {
						system.Render(system._engine->GetRenderingDevice());
					...
					if (!alive) {
						...
						system.SetRenderingDevice(system._engine->GetRenderingDevice());
		...
		void ControlSystem::ControlSystemWindowCallback::SetCursor(Windows::IWindow * window, UI::Point at) {}
		void ControlSystem::ControlSystemWindowCallback::MouseMove(Windows::IWindow * window, UI::Point at)
		{
		...
		void ControlSystem::ControlSystemWindowCallback::LeftButtonDown(Windows::IWindow * window, UI::Point at)
		{
			if (system._captured && !system._captured->IsAccessible()) system.SetCapture(0);
			Control * responder; Point lp;
			if (system._captured) {
		...
		void ControlSystem::ControlSystemWindowCallback::LeftButtonUp(Windows::IWindow * window, UI::Point at)
		{
			if (system._captured && !system._captured->IsAccessible()) system.SetCapture(0);
			Control * responder; Point lp;
			if (system._captured) {
		...
		void ControlSystem::ControlSystemWindowCallback::LeftButtonDoubleClick(Windows::IWindow * window, UI::Point at)
		{
			if (system._captured && !system._captured->IsAccessible()) system.SetCapture(0);
			Control * responder; Point lp;
			if (system._captured) {
		...
		void ControlSystem::ControlSystemWindowCallback::RightButtonDown(Windows::IWindow * window, UI::Point at)
		{
			if (system._captured && !system._captured->IsAccessible()) system.SetCapture(0);
			Control * responder; Point lp;
			if (system._captured) {
		...
		void ControlSystem::ControlSystemWindowCallback::RightButtonUp(Windows::IWindow * window, UI::Point at)
		{
			if (system._captured && !system._captured->IsAccessible()) system.SetCapture(0);
			Control * responder; Point lp;
			if (system._captured) {
		...
		void ControlSystem::ControlSystemWindowCallback::RightButtonDoubleClick(Windows::IWindow * window, UI::Point at)
		{
			if (system._captured && !system._captured->IsAccessible()) system.SetCapture(0);
			Control * responder; Point lp;
			if (system._captured) {
		...
		void ControlSystem::ControlSystemWindowCallback::Timer(Windows::IWindow * window, int timer_id)
		{
			if (timer_id == 1) {
				...
				auto control = system._timers.ElementByKey(timer_id);
		...
		ControlSystem::ControlSystem(void) : _animations(0x10), _cursors(0x10), _preferred_device_class(Windows::DeviceClass::Null), _caret_width(int(CurrentScaleFactor)), _virtual_size(1, 1), _autosize(false), _beep(true), _event_callback(0), _window(0), _timers(0x20),
		...
		void ControlSystem::ResetEffectiveRefreshRate(void)
		{
			...
			else if (_focus_rate == ControlRefreshPeriod::CaretBlink || _general_rate == ControlRefreshPeriod::CaretBlink) { frame_time = _device ? _device->GetCaretBlinkHalfTime() : 500; _evaluated_rate = ControlRefreshPeriod::CaretBlink; }
		...
		void ControlSystem::ResetRenderingDevice(void)
		{
			...
			if (_preferred_device_class != Windows::DeviceClass::Null) {
				...
				SetRenderingDevice(_engine->GetRenderingDevice());
			}
		...
		void ControlSystem::SetRenderingDevice(IRenderingDevice * device) { _device.SetRetain(device); }
		IRenderingDevice * ControlSystem::GetRenderingDevice(void) { return _device; }
			...
			if (period) {
				...
				if (_timers.Length()) {
					for (int i = 0; i < _timers.Length(); i++) {
						if (id == -1) {
							if (i) {
								if (_timers.ElementByIndex(i).key - _timers.ElementByIndex(i - 1).key > 1) id = _timers.ElementByIndex(i - 1).key + 1;
							} else {
								if (_timers.ElementByIndex(i).key > 2) id = 2;
							}
						}
						if (_timers.ElementByIndex(i).object.Inner() == control) { entry = i; break; }
					}
				} else id = 2;
				if (entry == -1) _timers.Append(id, control);
				else id = _timers.ElementByIndex(entry).key;
				...
				if (entry != -1) {
					_window->SetTimer(_timers.ElementByIndex(entry).key, 0);
					_timers.RemoveAt(entry);
				}
			}
		}
		void ControlSystem::Invalidate(void) { _window->InvalidateContents(); }
		void ControlSystem::Render(IRenderingDevice * device)
		{
			auto moment = GetTimerValue();
			device->SetTimerValue(moment);
		...
		void ParentControl::Render(IRenderingDevice * device, const Box & at)
		{
		...
		void RootControl::Render(IRenderingDevice * device, const Box & at)
		{
		...
		Windows::IMenu * CreateMenu(Template::ControlTemplate * base)
		{
			...
			public:
				...
				virtual Point MeasureMenuItem(Windows::IMenuItem * item, IRenderingDevice * device) override
				{
					auto props = static_cast<Template::Controls::MenuItem *>(Template->Properties);
					if (!props->Font) return Point(1, 1);
					SafePointer<ITextRenderingInfo> text = device->CreateTextRenderingInfo(props->Font, item->GetText(), 0, 0, 0);
					SafePointer<ITextRenderingInfo> right = device->CreateTextRenderingInfo(props->Font, item->GetSideText(), 0, 0, 0);
					int x1, x2, y;
					text->GetExtent(x1, y);
					right->GetExtent(x2, y);
					return Point(3 * props->Height + x1 + x2, props->Height);
				}
				virtual void RenderMenuItem(Windows::IMenuItem * item, IRenderingDevice * device, const Box & at, bool hot_state) override
				{
					auto props = static_cast<Template::Controls::MenuItem *>(Template->Properties);
					Shape ** _view;
					Template::Shape * View;
			...
			public:
				SafePointer<Template::ControlTemplate> Template;
				virtual Point MeasureMenuItem(Windows::IMenuItem * item, IRenderingDevice * device) override { return Point(1, static_cast<Template::Controls::MenuSeparator *>(Template->Properties)->Height); }
				virtual void RenderMenuItem(Windows::IMenuItem * item, IRenderingDevice * device, const Box & at, bool hot_state) override
				{
					if (!_shape) {
						ZeroArgumentProvider provider;
