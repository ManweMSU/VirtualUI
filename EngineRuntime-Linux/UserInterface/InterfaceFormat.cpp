			...
			void InterfaceTemplateImage::Compile(InterfaceTemplate & Template, IResourceLoader * ResourceLoader, IResourceResolver * ResourceResolver, IMissingStylesReporter * StyleReporter)
			{
				...
				for (int a = 0; a < Assets.Length(); a++) {
					...
					for (int i = 0; i < asset.Fonts.Length(); i++) {
						...
						SafePointer<IFont> font = Loader->LoadFont(asset.Fonts[i].FontFace, asset.Fonts[i].Height.Absolute + int(asset.Fonts[i].Height.Scalable * CurrentScaleFactor), asset.Fonts[i].Weight, asset.Fonts[i].IsItalic, asset.Fonts[i].IsUnderline, asset.Fonts[i].IsStrikeout);
