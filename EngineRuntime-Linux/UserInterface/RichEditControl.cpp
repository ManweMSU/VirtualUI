		...
		namespace Controls
		{
			namespace RichEditContents
			{
				...
				public:
					...
					{
						if (!_texture) {
							if (_image) _texture = device->LoadTexture(_image->GetFrameBestDpiFit(CurrentScaleFactor));
			...
			void RichEdit::_align_contents(IRenderingDevice * device, const Box & box)
			{
				if (_init) return;
			...
			void RichEdit::Render(IRenderingDevice * device, const Box & at)
			{
				device->PushClip(at);
				if (_init) {
			...
			void RichEdit::RecreateCachedFont(int index)
			{
				...
				_fonts[index].font = device->LoadFont(_font_faces[_fonts[index].face], int(_fonts[index].height * CurrentScaleFactor), weight, italic, underline, strikeout);
			}
