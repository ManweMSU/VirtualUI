...
namespace Engine
{
	namespace UI
	{
		namespace Controls
		{
			...
			Static::~Static(void) {}
			void Static::Render(IRenderingDevice * device, const Box & at)
			{
				if (!shape) {
					if (View) {
			...
			void Static::SetImage(ITexture * image) { Image.SetRetain(image); ResetCache(); Invalidate(); }
			...
			void ProgressBar::Render(IRenderingDevice * device, const Box & at)
			{
				if (!shape) {
					if (View) {
			...
			void ColorView::Render(IRenderingDevice * device, const Box & at)
			{
				if (!shape) {
					if (View) {
			...
			void ColorView::SetColor(UI::Color color) { Color = color; ResetCache(); Invalidate(); }
			...
			void ComplexView::Render(IRenderingDevice * device, const Box & at)
			{
				if (!shape) {
					if (View) {
			...
			void ComplexView::SetColor(UI::Color value, int index) { if (index == 0) Color = value; else if (index == 1) Color2 = value; ResetCache(); Invalidate(); }
			...
			void ComplexView::SetTexture(ITexture * value, int index) { if (index == 0) Image.SetRetain(value); else if (index == 1) Image2.SetRetain(value); ResetCache(); Invalidate(); }
			void ComplexView::SetFont(IFont * value, int index) { if (index == 0) Font.SetRetain(value); else if (index == 1) Font2.SetRetain(value); ResetCache(); Invalidate(); }
