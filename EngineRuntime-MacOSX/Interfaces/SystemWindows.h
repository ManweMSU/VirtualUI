...
namespace Engine
{
	namespace Windows
	{
		...
		struct CreateWindowDesc
		{
			...
			string Title;
			Box Position;
			Point MinimalConstraints;
			Point MaximalConstraints;
			Box FrameMargins;
		...
		public:
			...
			virtual Box GetScreenRectangle(void) noexcept = 0;
			virtual Box GetUserRectangle(void) noexcept = 0;
			virtual Point GetResolution(void) noexcept = 0;
		...
		public:
			...
			virtual void SetCursor(IWindow * window, Point at);
			virtual void MouseMove(IWindow * window, Point at);
			virtual void LeftButtonDown(IWindow * window, Point at);
			virtual void LeftButtonUp(IWindow * window, Point at);
			virtual void LeftButtonDoubleClick(IWindow * window, Point at);
			virtual void RightButtonDown(IWindow * window, Point at);
			virtual void RightButtonUp(IWindow * window, Point at);
			virtual void RightButtonDoubleClick(IWindow * window, Point at);
			// Scroll events
		...
		{
		public:
			virtual Point MeasureMenuItem(IMenuItem * item, Graphics::I2DDeviceContext * device) = 0;
			virtual void RenderMenuItem(IMenuItem * item, Graphics::I2DDeviceContext * device, const Box & at, bool hot_state) = 0;
		...
		public:
			...
			virtual Graphics::I2DDeviceContext * GetContext(void) noexcept = 0;
		...
		public:
			...
			virtual void SetPosition(const Box & box) = 0;
			virtual Box GetPosition(void) = 0;
			virtual Point GetClientSize(void) = 0;
			virtual void SetMinimalConstraints(Point size) = 0;
			virtual Point GetMinimalConstraints(void) = 0;
			virtual void SetMaximalConstraints(Point size) = 0;
			virtual Point GetMaximalConstraints(void) = 0;
			...
			virtual bool PointHitTest(Point at) = 0;
			virtual Point PointClientToGlobal(Point at) = 0;
			virtual Point PointGlobalToClient(Point at) = 0;
			...
			virtual void SetBackbufferedRenderingDevice(Codec::Frame * image, ImageRenderMode mode = ImageRenderMode::Stretch, Color filling = 0xFF000000) noexcept = 0;
		...
		public:
			...
			virtual int Run(IWindow * owner, Point at) noexcept = 0;
			virtual handle GetOSHandle(void) noexcept = 0;
		};
		...
		public:
			...
			virtual Point GetIconSize(void) noexcept = 0;
		...
		public:
			...
			// Measuring windows
			virtual Box ConvertClientToWindow(const Box & box, uint flags) noexcept = 0;
			virtual Point ConvertClientToWindow(const Point & size, uint flags) noexcept = 0;
			// Event handling
			...
			virtual Point GetCursorPosition(void) noexcept = 0;
			virtual void SetCursorPosition(Point position) noexcept = 0;
			...
			// Miscellaneous
			virtual Array<Point> * GetApplicationIconSizes(void) noexcept = 0;
			...
			virtual Point GetUserNotificationIconSize(void) noexcept = 0;
