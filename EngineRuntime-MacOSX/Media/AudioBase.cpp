...
namespace Engine
{
	namespace Audio
	{
		...
		bool IsSignedFormat(SampleFormat format) { return (reinterpret_cast<uint &>(format) & 0xFFFF0000) == 0x00010000; }
		bool IsUnsignedFormat(SampleFormat format) { return (reinterpret_cast<uint &>(format) & 0xFFFF0000) == 0x00030000; }
		...
		StreamDesc::operator string(void) const
		{
			...
			if (Format != SampleFormat::Invalid) {
				fmt = string(SampleFormatBitSize(Format)) + string(L"-bit ");
				if (IsSignedFormat(Format)) fmt += L"signed normalized integer";
				else if (IsUnsignedFormat(Format)) fmt += L"unsigned normalized integer";
				else if (IsFloatingPointFormat(Format)) fmt += L"floating point";
		...
		int32 WaveBuffer::ReadSampleInteger(uint64 frame_index, uint num_channel) const
		{
			...
			if (desc.Format == SampleFormat::S8_snorm || desc.Format == SampleFormat::S8_unorm) {
				uint32 result = uint32(*reinterpret_cast<uint8 *>(frame_buffer + sample_index)) << 24;
				if (IsUnsignedFormat(desc.Format)) result -= 0x80000000;
				result |= (result & 0x7F000000) >> 7;
				result |= (result & 0x7FFE0000) >> 14;
				result |= (result & 0x7FFFFFF8) >> 28;
				return result;
			} else if (desc.Format == SampleFormat::S16_snorm || desc.Format == SampleFormat::S16_unorm) {
				...
				return result;
			} else if (desc.Format == SampleFormat::S24_snorm || desc.Format == SampleFormat::S24_unorm) {
				...
				return result;
			} else if (desc.Format == SampleFormat::S32_snorm || desc.Format == SampleFormat::S32_unorm) {
				uint32 result = *reinterpret_cast<uint32 *>(frame_buffer + sample_index);
				if (IsUnsignedFormat(desc.Format)) result -= 0x80000000;
				return result;
		...
		double WaveBuffer::ReadSampleFloat(uint64 frame_index, uint num_channel) const
		{
			...
			if (desc.Format == SampleFormat::S8_snorm || desc.Format == SampleFormat::S8_unorm) {
				...
				if (desc.Format == SampleFormat::S8_unorm) return double(int8(int8(result) - int8(128))) / 128.0;
				else return double(result) / 128.0;
			} else if (desc.Format == SampleFormat::S16_snorm || desc.Format == SampleFormat::S16_unorm) {
				int32 result = *reinterpret_cast<int16 *>(frame_buffer + sample_index);
				if (desc.Format == SampleFormat::S16_unorm) return double(int16(int16(result) - int16(32768))) / 32768.0;
				else return double(result) / 32768.0;
			} else if (desc.Format == SampleFormat::S24_snorm || desc.Format == SampleFormat::S24_unorm) {
				...
				if (desc.Format == SampleFormat::S24_unorm) result += 0x80000000;
				result |= (result & 0x7FFFFF00) >> 23;
				return double(result) / 2147483648.0;
			} else if (desc.Format == SampleFormat::S32_snorm || desc.Format == SampleFormat::S32_unorm) {
				int32 result = *reinterpret_cast<int32 *>(frame_buffer + sample_index);
				if (desc.Format == SampleFormat::S32_unorm) result += 0x80000000;
				return double(result) / 2147483648.0;
			} else if (desc.Format == SampleFormat::S32_float) {
		...
		void WaveBuffer::WriteSample(uint64 frame_index, uint num_channel, int32 value)
		{
			intptr sample_index = intptr((frame_index * desc.ChannelCount + num_channel) * SampleFormatByteSize(desc.Format));
			if (desc.Format == SampleFormat::S8_snorm || desc.Format == SampleFormat::S8_unorm) {
				if (IsUnsignedFormat(desc.Format)) value += 0x80000000;
		...
		void WaveBuffer::WriteSample(uint64 frame_index, uint num_channel, double value)
		{
			...
			if (desc.Format == SampleFormat::S8_snorm || desc.Format == SampleFormat::S8_unorm) {
				...
				if (IsUnsignedFormat(desc.Format)) ivalue += 0x80000000;
