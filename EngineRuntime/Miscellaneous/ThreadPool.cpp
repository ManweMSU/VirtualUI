...
namespace Engine
{
	...
	ThreadPool::ThreadPool(int num_threads) : Workers(8), NumTasks(0)
	{
		...
		for (int i = 0; i < num_threads; i++) {
			...
			if (!worker) {
				BeginSubmit();
				for (int j = 0; j < i; j++) { TaskCount->Open(); Tasks.InsertLast(SafePointer<IDispatchTask>(0)); NumTasks++; }
				EndSubmit();
				Wait();
				throw Exception();
			}
	...
	ThreadPool::ThreadPool(int num_threads, uint32 stack_size) : Workers(8), NumTasks(0)
	{
		if (num_threads <= 0) num_threads = GetProcessorsNumber() + num_threads;
		if (num_threads <= 0) num_threads = 1;
		ActiveThreads = num_threads;
	...
	ThreadPool::~ThreadPool(void)
	{
		...
		for (int i = 0; i < Workers.Length(); i++) {
			TaskCount->Open();
			Tasks.InsertLast(SafePointer<IDispatchTask>(0));
			NumTasks++;
		}
		EndSubmit();
		Wait();
	...
	void Engine::ThreadPool::AppendTask(IDispatchTask * task)
	{
		...
		TaskCount->Open();
		try {
			SafePointer<IDispatchTask> wrapper;
			wrapper.SetRetain(task);
			Tasks.Push(wrapper);
			NumTasks++;
		} catch (...) { TaskCount->Wait(); throw; }
	}
	...
	void ThreadPool::Wait(void) const
	{
		...
		do {
			...
			Idle->Open();
			Access->Wait();
			if (!NumTasks && !ActiveThreads) idle = true;
			Access->Open();
		} while (!idle);
	}
	bool ThreadPool::Active(void) const
	{
		if (Idle->TryWait()) {
			Idle->Open();
			Access->Wait();
			bool idle = (!NumTasks && !ActiveThreads);
			Access->Open();
			return !idle;
		} else return true;
	}
	...
	{
		Access->Wait();
		string info = L"Thread Pool: " + string(Workers.Length()) + L" threads, " + string(NumTasks) + L" tasks pending, " + string(ActiveThreads) + L" threads active";
		Access->Open();
		return info;
	}
	...
	TaskQueue::TaskQueue(void) : IntTaskCount(0)
	{
		Access = CreateSemaphore(1);
		TaskCount = CreateSemaphore(0);
	...
	{
		Access->Wait();
		try { Tasks.Push(SafePointer<IDispatchTask>(0)); } catch (...) { Access->Open(); throw; }
		TaskCount->Open();
		IntTaskCount++;
		Access->Open();
	}
	...
	{
		Access->Wait();
		try { Tasks.InsertFirst(SafePointer<IDispatchTask>(0)); } catch (...) { Access->Open(); throw; }
		TaskCount->Open();
		IntTaskCount++;
		Access->Open();
	}
	...
	{
		Access->Wait();
		string info = L"Task Queue: " + string(IntTaskCount) + L" tasks pending";
		Access->Open();
		return info;
	}
