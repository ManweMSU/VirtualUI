...
#include "../PlatformDependent/SystemWindowsAPI.h"
...
namespace Engine
{
	...
	{
		NSImage * GetCocoaImage(Graphics::IBitmap * texture, const string & ident)
		{
			if (ident.Length()) {
				...
				CGImageRef image = reinterpret_cast<CGImageRef>(Cocoa::GetBitmapCoreImage(texture));
				CGSize size;
		...
		class TouchBarButtonImp : public TouchBarButton
		{
			...
			int _ID;
			string Text;
			SafePointer<Graphics::IBitmap> Image;
			string ImageID;
		public:
			...
			virtual Graphics::IBitmap * GetImage(void) override { return Image; }
			virtual void SetImage(Graphics::IBitmap * image) override
			{
				@autoreleasepool {
			...
			virtual Color GetColor(void) override { return Color(r, g, b, a); }
		...
		public:
			...
			void IntSetColor(Color color) { _Color = color; }
			virtual void WakeUp(void) override
			{
				@autoreleasepool {
					NSString * ident = Cocoa::CocoaString(Identifier);
			...
			virtual int GetChildrenCount(void) override { return 0; }
			virtual TouchBarItem * GetChild(int index) override { return 0; }
			virtual Color GetColor(void) override { return _Color; }
			virtual void SetColor(Color color) override
			{
				@autoreleasepool {
					_Color = color;
		...
		class TouchBarSliderImp : public TouchBarSlider
		{
			...
			double Position;
			int Ticks;
			SafePointer<Graphics::IBitmap> MinImage;
			SafePointer<Graphics::IBitmap> MaxImage;
			string MinImageID;
			string MaxImageID;
		public:
			...
			virtual Graphics::IBitmap * GetMinimalImage(void) override { return MinImage; }
			virtual void SetMinimalImage(Graphics::IBitmap * image) override
			{
				MinImage.SetRetain(image);
				if (item) { Shutdown(); WakeUp(); }   
			}
			virtual Graphics::IBitmap * GetMaximalImage(void) override { return MaxImage; }
			virtual void SetMaximalImage(Graphics::IBitmap * image) override
			{
		...
		TouchBar::TouchBar(void) : RootItems(0x10), Callback(0), MainItemID(0), WindowAttached(0)
		{
		...
		UI::IEventCallback * TouchBar::GetCallback(void) { return Callback; }
		void TouchBar::SetCallback(UI::IEventCallback * callback) { Callback = callback; }
		void * TouchBar::GetDelegate(void) { return Delegate; }
		Windows::IWindow * TouchBar::GetCurrentWindow(void) { return WindowAttached; }
		...
		void TouchBar::SetTouchBarForWindow(Windows::IWindow * window, TouchBar * bar)
		{
			if (bar) {
				if (bar->WindowAttached) throw InvalidStateException();
				[Cocoa::GetWindowObject(window) setTouchBar: ((NSTouchBar *) bar->Bar)];
				Cocoa::SetWindowTouchBar(window, bar);
				bar->WindowAttached = window;
			} else {
				auto current = GetTouchBarFromWindow(window);
				if (current) current->WindowAttached = 0;
				Cocoa::SetWindowTouchBar(window, 0);
			}
		}
		TouchBar * TouchBar::GetTouchBarFromWindow(Windows::IWindow * window) { return static_cast<TouchBar *>(Cocoa::GetWindowTouchBar(window)); }
		TouchBar * SetTouchBarFromTemplate(Windows::IWindow * window, UI::Template::ControlTemplate * source)
		{
...
{
	int ID = [sender tag];
	if (ID) bar->GetCallback()->HandleControlEvent(bar->GetCurrentWindow(), ID, Engine::UI::ControlEvent::MenuCommand, 0);
}
- (void) handle_touch_bar_event_from_picker: (id) sender
{
