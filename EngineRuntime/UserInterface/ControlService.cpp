...
namespace Engine
{
	namespace UI
	{
		...
		void MenuArgumentProvider::GetArgument(const string & name, Graphics::IBitmap ** value)
		{
		...
		void MenuArgumentProvider::GetArgument(const string & name, Graphics::IFont ** value)
		{
			...
			virtual Point MeasureMenuItem(Windows::IMenuItem * item, Graphics::I2DDeviceContext * device) override
			{
				if (_size.x < 0) {
					if (_item->IsSeparator()) {
						...
						SafePointer<Graphics::ITextBrush> info = device->CreateTextBrush(_styles->MenuItemReferenceFont, _item->GetText() + _item->GetSideText(), 0, 0, 0);
						int w, h;
						info->GetExtents(w, h);
		...
		public:
			...
			MenuList(void) : Layout(0), TotalHeight(0), TotalWidth(0), Current(0), Mode(0), Border(0), Submenu(0), SubmenuIndex(-1), Callbacks(0x20), Heights(0x20) {}
			...
			{
				if (Menu) {
					auto stl = Layout ? Layout->ProvideStyles() : GetControlSystem()->GetVirtualPopupStyles();
					...
					for (int i = 0; i < Menu->Length(); i++) {
						...
						if (menu_item->GetCallback()) {
							...
							SafePointer<VirtualMenuCallback> vmc = new VirtualMenuCallback(stl, menu_item);
							callback = vmc;
					...
					Current = -1;
					Border = stl->MenuBorderWidth;
					TotalHeight += 2 * Border;
					TotalWidth += 2 * Border;
				} else if (Layout) {
					auto box = Layout->ProvidePrimaryBox();
					TotalWidth = box.Right - box.Left;
					TotalHeight = box.Bottom - box.Top;
					Current = Layout->ProvidePrimarySubmenu();
					Border = 0;
					Mode = 1;
					ShowSubmenu();
				}
			...
			void ShowSubmenu()
			{
				...
				Submenu = submenu;
				Submenu->Menu.SetRetain(Menu ? Menu->ElementAt(Current)->GetSubmenu() : Layout->GetElementMenu(Current));
				Submenu->Initialize();
				if (Menu) {
					int y = 0;
					...
					Box at = Layout->ProvidePrimaryBox();
					Box rect = Layout->GetElementBox(Current);
					Box item = Box(at.Left + rect.Left, at.Top + rect.Top, at.Left + rect.Right, at.Top + rect.Bottom);
					Submenu->SetPosition(Box(item.Left, item.Bottom, item.Left + Submenu->TotalWidth, item.Bottom + Submenu->TotalHeight));
				}
				Invalidate();
			}
			...
			void RenderShadow(Graphics::I2DDeviceContext * device, const Box & at)
			{
				if (Menu) {
					auto stl = Layout ? Layout->ProvideStyles() : GetControlSystem()->GetVirtualPopupStyles();
					if (!_shadow && stl->PopupShadow) {
						_shadow = stl->PopupShadow->Clone();
					}
			...
			virtual void Render(Graphics::I2DDeviceContext * device, const Box & at) override
			{
				if (Menu) {
					auto stl = Layout ? Layout->ProvideStyles() : GetControlSystem()->GetVirtualPopupStyles();
					if (!_background && stl->MenuFrame) {
						_background = stl->MenuFrame->Clone();
					}
					...
					for (int i = 0; i < Menu->Length(); i++) {
						...
						if (!element->IsSeparator()) {
							if (element->GetSubmenu()) {
								if (!_arrow && stl->SubmenuArrow) {
									_arrow = stl->SubmenuArrow->Clone();
								}
								if (_arrow) _arrow->Render(device, item);
							}
						}
					}
				} else if (Layout) Layout->RenderMenu(device, at, Current);
			}
			...
			virtual void LeftButtonDown(Point at) override { if (!Menu && GetControlSystem()) GetControlSystem()->SetExclusiveControl(0); }
			...
			virtual void MouseMove(Point at) override
			{
				if (Menu) {
					if (Mode == 0) {
						if (at.x < Border || at.y < Border || at.x >= TotalWidth - Border || at.y >= TotalHeight - Border) {
							...
							if (SubmenuIndex != Current) {
								...
								if (!Menu->ElementAt(Current)->IsSeparator()) {
									auto item = Menu->ElementAt(Current);
									if (item->GetSubmenu() && item->IsEnabled()) ShowSubmenu();
								}
							}
						}
					}
					...
					for (int i = 0; i < Layout->GetElementCount(); i++) {
						if (Layout->GetElementMenu(i) && Layout->GetElementBox(i).IsInside(at)) { Current = i; Invalidate(); break; }
					}
					if (SubmenuIndex != Current) {
						CloseSubmenu();
						auto menu = Layout->GetElementMenu(Current);
						if (menu) ShowSubmenu();
					}
				}
			}
		...
		public:
			...
			MenuHolder(void) : _client(-1, -1, -1, -1), Layout(0), List(0), Final(false) {}
			virtual ~MenuHolder(void) override {}
			virtual void Render(Graphics::I2DDeviceContext * device, const Box & at) override
			{
				Shape * client;
				if (Layout && (client = Layout->ProvideClientBackground())) {
					if (_client.Left < 0) _client = Layout->ProvideClientBox();
					client->Render(device, _client);
				}
		...
		void RunVirtualMenu(Control * for_control, IMenuLayoutProvider * provider)
		{
			...
			system->SetFocus(holder);
			SafePointer<MenuList> list = new MenuList;
			list->Layout = provider;
			holder->AddChild(list);
			list->Initialize();
			list->SetPosition(provider->ProvidePrimaryBox());
			holder->Invalidate();
		}
