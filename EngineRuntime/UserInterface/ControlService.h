...
namespace Engine
{
	namespace UI
	{
		...
		public:
			...
			SafePointer<Graphics::IFont> MenuItemReferenceFont;
		...
		class IMenuLayoutProvider
		{
		public:
			virtual VirtualPopupStyles * ProvideStyles(void) = 0;
			virtual Box ProvidePrimaryBox(void) = 0;
			virtual Box ProvideClientBox(void) = 0;
			virtual Shape * ProvideClientBackground(void) = 0;
			virtual int ProvidePrimarySubmenu(void) = 0;
			virtual void RenderMenu(Graphics::I2DDeviceContext * device, const Box & at, int current) = 0;
			virtual int GetElementCount(void) = 0;
			virtual Windows::IMenu * GetElementMenu(int index) = 0;
			virtual Box GetElementBox(int index) = 0;
		};
	}
}